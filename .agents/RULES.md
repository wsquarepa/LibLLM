...
- **`commands`** -- Shared command registry for `/help` and TUI command picker; includes `resolve_alias()` and `matching_commands()`. Commands: `/clear` (`/new`), `/system`, `/retry`, `/continue` (`/cont`), `/branch`, `/character`, `/persona` (`/self`, `/user`, `/me`), `/worldbook` (`/lore`, `/world`, `/lorebook`), `/passkey` (`/password`, `/pass`, `/auth`), `/config`, `/theme`, `/export`, `/macro`, `/report`, `/quit` (`/exit`)
