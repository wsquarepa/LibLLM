...
use backup::index::{BackupEntry, BackupType, load_index, parse_backup_filename, save_index, BackupIndex};
use backup::verify::verify_chain;
use backup::restore::restore_to_point;

#[derive(Parser)]
#[command(name = "libllm-recover", about = "Backup recovery tool for libllm databases")]
struct Args {
    #[arg(short = 'd', long)]
    data: PathBuf,
...
fn cmd_list(data: &PathBuf) -> Result<()> {
    let index_path = data.join("backups").join("index.json");
...
fn cmd_verify(data: &PathBuf, passkey: Option<&str>, full: bool) -> Result<()> {
    let result = verify_chain(data, passkey, full)?;
...
fn cmd_restore(data: &PathBuf, passkey: Option<&str>, id: &str, yes: bool) -> Result<()> {
    let index_path = data.join("backups").join("index.json");
    let index = load_index(&index_path)?;
    ...
    restore_to_point(data, id, passkey)?;
    println!("Restore to '{id}' completed successfully.");
    Ok(())
}

fn cmd_rebuild_index(data: &PathBuf, passkey: Option<&str>) -> Result<()> {
    ...
    let backup_key: Option<[u8; 32]> = match passkey {
        Some(pk) => {
            let salt = libllm::crypto::load_or_create_salt(&data.join(".salt"))?;
    ...
    for file in &files {
        ...
        // Assign base_id for diffs: the most recent base entry before this diff.
...
fn main() -> Result<()> {
    let args = Args::parse();
    let passkey = args.passkey.as_deref();

    match args.command {
        Command::List => cmd_list(&args.data),
        Command::Verify { full } => cmd_verify(&args.data, passkey, full),
        Command::Restore { id, yes } => cmd_restore(&args.data, passkey, &id, yes),
        Command::RebuildIndex => cmd_rebuild_index(&args.data, passkey),
    }
}
