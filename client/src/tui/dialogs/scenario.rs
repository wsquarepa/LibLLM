...
fn dialog_rect(area: Rect) -> Rect {
    ...
    crate::tui::render::centered_rect(width, height, area)
}

pub(in crate::tui) fn render(f: &mut Frame, app: &mut App, area: Rect) {
...
pub(in crate::tui) fn insert_text(app: &mut App, text: &str) {
    if let Some(ref mut editor) = app.scenario_editor {
        editor.insert_str(text);
    }
}

pub(in crate::tui) fn scroll_by(app: &mut App, rows: i16) -> bool {
    if let Some(ref mut editor) = app.scenario_editor {
...
pub(in crate::tui) fn handle_mouse_click(app: &mut App, screen_col: u16, screen_row: u16) {
...
pub(in crate::tui) fn handle_mouse_drag(app: &mut App, screen_col: u16, screen_row: u16) {
    let Some(ref mut editor) = app.scenario_editor else {
        return;
    };
    let Ok((tw, th)) = crossterm::terminal::size() else {
