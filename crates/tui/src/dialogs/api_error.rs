...
pub(crate) fn render_api_error_dialog(f: &mut ratatui::Frame, app: &App, area: Rect) {
...
pub(crate) fn handle_api_error_key(key: KeyEvent, app: &mut App) -> Option<Action> {
    match key.code {
...
pub(crate) fn render_loading_dialog(f: &mut ratatui::Frame, area: Rect) {
    let dialog = clear_centered(
        f,
...
pub(crate) fn handle_loading_key(key: KeyEvent) -> Option<Action> {
