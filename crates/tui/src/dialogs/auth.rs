...
pub(crate) enum AuthDialogAction {
    Continue,
    Close,
    OpenTypePicker,
}
...
pub(crate) fn open_auth_dialog(app: &mut App) {
...
pub(crate) fn render_auth_dialog(f: &mut ratatui::Frame, app: &App, area: Rect) {
    ...
    render_hints_below_dialog(
        ...
        area,
        &[Line::from(
            "Up/Down: navigate  Enter: edit/select  Ctrl+S: save  Esc: close",
        )],
    );
}
...
pub(crate) fn close_and_persist(app: &mut App) {
    ...
    crate::business::apply_config(app);
...
pub(crate) fn open_type_picker(app: &mut App) {
    let Some(state) = app.auth_dialog.as_mut() else {
        return;
    };
...
pub(crate) fn render_type_picker(f: &mut ratatui::Frame, app: &App, area: Rect) {
    let Some(state) = app.auth_dialog.as_ref() else {
        return;
    };
...
pub(crate) fn handle_type_picker_key(key: KeyEvent, app: &mut App) -> Option<crate::types::Action> {
    let Some(state) = app.auth_dialog.as_mut() else {
        app.focus = Focus::ConfigDialog;
        return None;
    };
