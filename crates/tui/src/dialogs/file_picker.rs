...
pub(crate) const FILE_PICKER_DIALOG_WIDTH: u16 = 80;
pub(crate) const FILE_PICKER_DIALOG_HEIGHT: u16 = 20;
...
pub(crate) fn render(f: &mut ratatui::Frame, app: &mut App, area: Rect) {
...
fn accept_current(app: &mut App) {
    ...
    let Some(replacement) = crate::events::format_at_token(&full_path) else {
        app.set_status(
            format!("Cannot attach file: path contains quote character: {full_path}"),
            crate::StatusLevel::Warning,
        );
        app.file_picker = None;
        return_to_input(app);
        return;
