...
pub(crate) use crypto::derive_key_blocking;
use crypto::log_phase_with_path;
pub(crate) use mouse::handle_dialog_mouse_click;
pub(crate) use paged_list::{
...
pub(crate) fn byte_pos_at_char(s: &str, char_idx: usize) -> usize {
    s.char_indices()
        .nth(char_idx)
        .map(|(i, _)| i)
        .unwrap_or(s.len())
...
pub(crate) fn multiline_editor_content_rect(dialog: ratatui::layout::Rect) -> Rect {
    Rect {
...
pub(crate) fn render_multiline_editor(
    f: &mut ratatui::Frame,
    ...
    f.render_widget(
        crate::render::dialog_block(title, ratatui::style::Color::Yellow),
        dialog,
    );
    crate::render::render_hints_below_dialog(
        f,
        dialog,
        area,
...
pub(crate) const FIELD_DIALOG_PADDING_ROWS: u16 = 3;
...
impl<'a> FieldDialog<'a> {
    ...
    pub(crate) fn with_locked_fields(mut self, fields: Vec<usize>) -> Self {
    ...
    fn render_with_editor(&self, f: &mut ratatui::Frame, dialog: Rect, area: Rect) {
        ...
        let editor_area = multiline_editor_content_rect(dialog);
        let new_top =
            crate::events::update_scroll_top(self.editor_scroll_top.get(), editor, editor_area);
