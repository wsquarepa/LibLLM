...
pub(crate) fn render_regex_dialog(f: &mut ratatui::Frame, app: &App, area: Rect) {
...
pub(crate) fn handle_regex_editor_key(key: KeyEvent, app: &mut App) -> Option<Action> {
    ...
    match key.code {
        KeyCode::Esc => {
            ...
            if dirty {
                app.unsaved_warning =
                    Some(crate::dialogs::unsaved_warning::UnsavedWarningState::new(
...
fn commit_editor(app: &mut App) {
    ...
    if let Some(msg) = validate_pattern(&mut ed.draft) {
        app.set_status(
            ...
            crate::types::StatusLevel::Warning,
        );
    }
