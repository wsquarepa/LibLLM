...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum UnsavedOutcome {
    Pending,
    Chosen(UnsavedButton),
}

pub(crate) struct UnsavedWarningState {
    pub(crate) focused: UnsavedButton,
...
impl UnsavedWarningState {
    pub(crate) fn new(return_focus: Focus) -> Self {
        Self {
...
pub(crate) fn handle_key(state: &mut UnsavedWarningState, key: KeyEvent) -> UnsavedOutcome {
    match key.code {
        KeyCode::Left => {
