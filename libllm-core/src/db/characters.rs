...
pub fn insert_character(conn: &Connection, slug: &str, card: &CharacterCard) -> Result<()> {
    ...
    let alternate_greetings =
        serde_json::to_string(&card.alternate_greetings).context("failed to serialize alternate_greetings")?;
    conn.execute(
...
pub fn load_character(conn: &Connection, slug: &str) -> Result<CharacterCard> {
    ...
    .and_then(
        |(
            ...
            let alternate_greetings: Vec<String> = serde_json::from_str(&alternate_greetings_json)
