...
use libllm::character::{CharacterCard, extract_png_card, parse_card_json};
use libllm::persona::PersonaFile;
use libllm::session::Session;
use libllm::system_prompt::SystemPromptFile;
use libllm::worldinfo::WorldBook;
        ...
        match read_and_decode(&path, key_bytes) {
            Ok(json) => match serde_json::from_str::<WorldBook>(&json) {
                ...
                Err(_) => {
                    ...
                    match libllm::worldinfo::parse_worldbook_json(&json, &fallback_name) {
                        Ok(wb) => results.push((slug, wb)),
                        Err(e) => eprintln!(
